...
type ModalStep = 'MAIN' | 'ACCOUNT_PICKER' | 'CONFIRM_ACCOUNT';
...
export const GoogleLoginModal: React.FC<GoogleLoginModalProps> = ({
  ...
  // Manual Custom Input State
  const [emailInput, setEmailInput] = useState('');
  const [nameInput, setNameInput] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isDarkMode, setIsDarkMode] = useState(true);

  // Preset Active Accounts on Device (Simulating Active OAuth Device Accounts)
  const devicePresetAccounts: CustomerUser[] = [
    {
      id: 'cust-demo-1',
      googleId: 'g-thoriq01',
      name: 'Thoriq Agil',
      email: 'thoriqagil01@gmail.com',
      phone: '085113661387',
      avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=thoriqagil01@gmail.com',
    },
    {
      id: 'cust-demo-2',
      googleId: 'g-kedai03',
      name: 'Pelanggan Kedai',
      email: 'kedainyamleng03@gmail.com',
      phone: '085113661387',
      avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=kedainyamleng03@gmail.com',
    },
  ];
  ...
  // Open Account Picker Sheet for Google or Apple
  const handleOpenAccountPicker = (provider: AuthProvider) => {
    setActiveProvider(provider);
    setModalStep('ACCOUNT_PICKER');
  };

  // Select Account from Picker Sheet and proceed to Confirmation Step
  const handleSelectAccountFromPicker = (user: CustomerUser) => {
  ...
  // Custom Email Input Form Submit
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in pb-[env(safe-area-inset-bottom)]">
      ...
      >
        ...
        {/* Theme Switcher & Back Navigation Bar */}
        ...
        {modalStep === 'MAIN' && (
          <div className="space-y-6 relative z-10 animate-fade-in">
            ...
            {/* 1. Primary Button: Continue with Google (Triggers Account Picker) */}
            <div className="space-y-3">
              <button
                type="button"
                onClick={() => handleOpenAccountPicker('GOOGLE')}
              ...
              {/* 2. Secondary Button: Continue with Apple (Triggers Account Picker) */}
              <button
                type="button"
                onClick={() => handleOpenAccountPicker('APPLE')}
                className={`w-full py-3.5 px-4 text-xs sm:text-sm font-bold rounded-2xl border shadow-md flex items-center justify-center gap-3 transition-all cursor-pointer active:scale-98 ${
                  isDarkMode
                    ? 'bg-[#27272a] hover:bg-[#3f3f46] text-white border-white/15'
        ...
        {/* STEP 2: INTERACTIVE ACCOUNT PICKER SHEET */}
        {modalStep === 'ACCOUNT_PICKER' && (
          <div className="space-y-5 relative z-10 animate-fade-in">
            <div className="text-center space-y-1">
              <div className="w-10 h-10 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center mx-auto mb-2">
                <UserCheck className="w-5 h-5" />
              </div>
              <h3 className={`text-lg font-black tracking-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                Pilih Akun {activeProvider === 'GOOGLE' ? 'Google' : 'Apple ID'}
              </h3>
              <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Pilih akun yang berjalan pada perangkat Anda untuk melanjutkan.
              </p>
            </div>

            {/* Account List */}
            <div className="space-y-2.5">
              <span className={`text-[10px] font-extrabold uppercase tracking-wider block ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                Akun Terdeteksi di Perangkat:
              </span>

              {devicePresetAccounts.map((acc) => (
                <button
                  key={acc.id}
                  type="button"
                  onClick={() => handleSelectAccountFromPicker(acc)}
                  className={`w-full p-3.5 rounded-2xl border text-left flex items-center justify-between gap-3 transition-all cursor-pointer active:scale-98 group ${
                    isDarkMode 
                      ? 'bg-[#222226] hover:bg-[#2e2e33] border-white/10 hover:border-amber-500/50' 
                      : 'bg-gray-50 hover:bg-amber-50/60 border-gray-200 hover:border-amber-400'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <img
                      src={acc.avatarUrl}
                      alt={acc.name}
                      className="w-10 h-10 rounded-full bg-amber-500/20 shrink-0 border border-white/20"
                    />
                    <div className="min-w-0">
                      <h4 className={`font-bold text-xs truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                        {acc.name}
                      </h4>
                  ...
                  <ChevronRight className="w-4 h-4 text-gray-400 group-hover:translate-x-0.5 group-hover:text-amber-500 transition-all shrink-0" />
                </button>
              ))}
            </div>

            {/* Manual Email Input Section */}
            <div className={`pt-3 border-t ${isDarkMode ? 'border-white/10' : 'border-gray-200'} space-y-3`}>
              <span className={`text-[10px] font-extrabold uppercase tracking-wider block ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                Atau Gunakan Email Google/Apple Lainnya:
              </span>
              ...
              <form onSubmit={handleEmailFormSubmit} className="space-y-3">
                <input
                  ...
                  className={`w-full px-4 py-2.5 border rounded-xl text-xs font-semibold focus:outline-none focus:border-amber-500 transition-all ${
                ...
                <button
                  type="submit"
                  disabled={!emailInput}
                  className="w-full py-3 px-4 bg-amber-500 hover:bg-amber-600 active:scale-98 text-charcoal font-bold text-xs rounded-xl shadow-md flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50"
                >
                  <Plus className="w-4 h-4" />
                  <span>Pilih &amp; Gunakan Email Ini</span>
                </button>
              </form>
            </div>
          </div>
        ...
        {modalStep === 'CONFIRM_ACCOUNT' && selectedUser && (
          <div className="space-y-5 relative z-10 animate-fade-in text-center">
            ...
            <div className="space-y-2">
              ...
              <button
                type="button"
                onClick={() => setModalStep('ACCOUNT_PICKER')}
                className={`text-xs font-bold underline ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-black'}`}
              >
                Pilih Akun Lain
              </button>
            </div>
